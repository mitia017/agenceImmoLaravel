...
@section('content')
    <div class="w-full mx-auto px-8 py-6  bg-gray-100 dark:bg-gray-900">
        @if (session('success'))
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="{{ route('admin.property.create') }}"
                class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg shadow transition">Ajouter un nouveau
                bien
            </a>
        </div>
                ...
                <tbody class="divide-y">
                    @foreach ($properties as $property)
                        <tr class="hover:bg-gray-500 transition">
                            <td class="px-6 py-4 text-white">{{ $property->title }}</td>
                            ...
                            <td class="px-6 py-4 text-right space-x-2">
                                <a href="{{ route('admin.property.edit', $property) }}"
                                    class="inline-block bg-blue-400 hover:bg-blue-600 text-white px-4 py-2 rounded">
                                    Éditer
                                </a>

                                <form action="{{ route('admin.property.destroy', $property) }}" method="POST"
                                    class="inline-block delete-form" id="{{ $property->id }}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="bg-red-400 hover:bg-red-600 text-white px-4 py-2 rounded">
                                        Supprimer
                                    </button>
                                </form>
                            </td>
